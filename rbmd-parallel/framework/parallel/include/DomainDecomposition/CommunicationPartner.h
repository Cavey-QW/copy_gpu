...
public:
    CommunicationPartner(int r, const double hLo[3], const double hHi[3], const double bLo[3],
                         const double bHi[3], const double sh[3], const int offset[3], const bool enlarged[3][2]);
    explicit CommunicationPartner(int r);
    CommunicationPartner(int r, const double leavingLo[3], const double leavingHi[3]);
    ...
    void initSend(LinkedCell* moleculeContainer, const MPI_Comm& comm, const MPI_Datatype& type,
                  MessageType msgType, std::vector<Atom>& invalidParticles, bool mightUseInvalidParticles,
    ...
    bool testRecv(LinkedCell* moleculeContainer, bool removeRecvDuplicates, bool force = false);
...
private:
    ...
    void collectMoleculesInRegion(LinkedCell* moleculeContainer, const double lowCorner[3],
    ...
    void collectLeavingMoleculesFromInvalidParticles(std::vector<Atom>& invalidParticles, double lowCorner [3], double highCorner [3], double shift [3]);
