...
// size_t CommunicationBuffer::_numBytesLeaving = 6 * sizeof(vcp_real_calc) +
...
//	#endif
//		;
// size_t CommunicationBuffer::_numBytesForces = sizeof(unsigned long) + 3 *
// sizeof(vcp_real_calc) + 3 * sizeof(vcp_real_accum); #else
//  position, velocity, orientation, angular momentum, id, cid
size_t CommunicationBuffer::_numBytesLeaving =
    7 * sizeof(Real_mpi) + sizeof(unsigned long);
// position, orientation, id, cid
size_t CommunicationBuffer::_numBytesHalo =
    3 * sizeof(Real_mpi)  //+ sizeof(int)
#ifdef LS1_SEND_UNIQUE_ID_FOR_HALO_COPIES
    + sizeof(unsigned long)
#endif
    ;
size_t CommunicationBuffer::_numBytesForces =
    sizeof(unsigned long) + 12 * sizeof(double);
// #endif
...
void CommunicationBuffer::resizeForReceivingMolecules(unsigned long &numLeaving,
                                                      unsigned long &numHalo) {
  // adjust for force exchange?
...
void CommunicationBuffer::resizeForReceivingMolecules(
...
void CommunicationBuffer::resizeForAppendingLeavingMolecules(
    unsigned long numLeaving) {
  _numLeaving += numLeaving;
  rbmd_assert(_numHalo ==
              0ul);  // assumption: add leaving, add leaving, then add halo,
  // halo, halo, ... but not intertwined.
  size_t numBytes = sizeof(_numHalo) + sizeof(_numLeaving) +
                    _numLeaving * _numBytesLeaving + _numHalo * _numBytesHalo;
...
void CommunicationBuffer::resizeForAppendingHaloMolecules(
    unsigned long numHalo) {
  // _numLeaving stays
  _numHalo += numHalo;
  size_t numBytes = sizeof(_numHalo) + sizeof(_numLeaving) +
...
void CommunicationBuffer::resizeForAppendingForceMolecules(
    unsigned long numForces) {
  ...
  // Do NOT write the number of force molecules, here! It is assumed at other
  // places, that they are NOT exchanged!
}

void CommunicationBuffer::addLeavingMolecule(size_t indexOfMolecule,
                                             const Atom &m) {
  rbmd_assert(indexOfMolecule < _numLeaving);

  size_t i_firstByte =
      getStartPosition(ParticleType_t::LEAVING, indexOfMolecule);
  rbmd_assert(i_firstByte + _numBytesLeaving <= _buffer.capacity());

  size_t i_runningByte = i_firstByte;
  // #ifdef ENABLE_REDUCED_MEMORY_MODE
  //     // add id, r, v
  //	i_runningByte = emplaceValue(i_runningByte, m.getID());
  //	i_runningByte = emplaceValue(i_runningByte,
  // static_cast<vcp_real_calc>(m.r(0))); 	i_runningByte =
  // emplaceValue(i_runningByte, static_cast<vcp_real_calc>(m.r(1)));
  //	i_runningByte = emplaceValue(i_runningByte,
  // static_cast<vcp_real_calc>(m.r(2))); 	i_runningByte =
  // emplaceValue(i_runningByte, static_cast<vcp_real_calc>(m.v(0)));
  //	i_runningByte = emplaceValue(i_runningByte,
  // static_cast<vcp_real_calc>(m.v(1))); 	i_runningByte =
  // emplaceValue(i_runningByte, static_cast<vcp_real_calc>(m.v(2))); #else
  i_runningByte = emplaceValue(i_runningByte, m.GetID());
  ...
  i_runningByte = emplaceValue(i_runningByte, m._charge);
  ...
  rbmd_assert(i_runningByte - i_firstByte == _numBytesLeaving);
}

void CommunicationBuffer::addHaloMolecule(size_t indexOfMolecule,
                                          const Atom &m) {
  rbmd_assert(indexOfMolecule < _numHalo);

  size_t i_firstByte = getStartPosition(ParticleType_t::HALO, indexOfMolecule);
  rbmd_assert(i_firstByte + _numBytesHalo <= _buffer.capacity());
  ...
  // emplaceValue(i_runningByte, static_cast<vcp_real_calc>(m.r(1)));
  // i_runningByte = emplaceValue(i_runningByte,
  // static_cast<vcp_real_calc>(m.r(2))); #else
#ifdef LS1_SEND_UNIQUE_ID_FOR_HALO_COPIES
  i_runningByte = emplaceValue(i_runningByte, m.GetID());  //+8 u long
#endif /*LS1_SEND_UNIQUE_ID_FOR_HALO_COPIES*/
  //    i_runningByte = emplaceValue(i_runningByte, m.componentid());
  i_runningByte = emplaceValue(i_runningByte, m.r(0));  // +4
  i_runningByte = emplaceValue(i_runningByte, m.r(1));
  i_runningByte = emplaceValue(
      i_runningByte,
      m.r(2));  // TODO 2024年4月2日 急 确定要什么属性 然后保证发送读取一致性
  //    i_runningByte = emplaceValue(i_runningByte, m.q().qw());
  //    i_runningByte = emplaceValue(i_runningByte, m.q().qx());
  //    i_runningByte = emplaceValue(i_runningByte, m.q().qy());
  //    i_runningByte = emplaceValue(i_runningByte, m.q().qz());
  ...
  if (i_runningByte - i_firstByte != _numBytesHalo) {
    GlobalLogger::error("i_runningByte - i_firstByte != _numBytesHalo");
  }
}

void CommunicationBuffer::addForceMolecule(size_t indexOfMolecule,
                                           const Atom &m) {
  // some MarDynAssert?
  size_t i_firstByte = getStartPosition(
      ParticleType_t::FORCE, indexOfMolecule);  // adjust getStartPosition etc.
  // some MarDynAssert?
  ...
  //	i_runningByte = emplaceValue(i_runningByte,
  // static_cast<vcp_real_calc>(m.r(2))); 	i_runningByte =
  // emplaceValue(i_runningByte, static_cast<vcp_real_accum>(m.F(0)));
  //	i_runningByte = emplaceValue(i_runningByte,
  // static_cast<vcp_real_accum>(m.F(1))); 	i_runningByte =
  // emplaceValue(i_runningByte, static_cast<vcp_real_accum>(m.F(2))); #else
  i_runningByte = emplaceValue(i_runningByte, m.GetID());
  i_runningByte = emplaceValue(i_runningByte, m.r(0));
  i_runningByte = emplaceValue(i_runningByte, m.r(1));
  i_runningByte = emplaceValue(i_runningByte, m.r(2));
...
void CommunicationBuffer::readLeavingMolecule(size_t indexOfMolecule,
                                              Atom &m) const {
  rbmd_assert(indexOfMolecule < _numLeaving);
  ...
  unsigned long idbuf;
  unsigned int cidbuf;
  double rbuf[3], vbuf[3], chargebuf;
  i_runningByte = readValue(i_runningByte, idbuf);
  ...
  i_runningByte = readValue(i_runningByte, vbuf[1]);
  i_runningByte = readValue(i_runningByte, vbuf[2]);
  i_runningByte = readValue(i_runningByte, chargebuf);
  ...
  //    TODO What m.setComponent(component);
  m = Atom(
      idbuf, rbuf[0], rbuf[1], rbuf[2], vbuf[0], vbuf[1],
      vbuf[2],chargebuf  // TODO 2024年4月11日 是否只要位置就可以呢  急  否！2024-07-27
  );
  // #endif
...
void CommunicationBuffer::readHaloMolecule(size_t indexOfMolecule,
                                           Atom &m) const {
  rbmd_assert(indexOfMolecule < _numHalo);
  size_t i_firstByte = getStartPosition(ParticleType_t::HALO, indexOfMolecule);
  rbmd_assert(i_firstByte + _numBytesHalo <= _buffer.capacity());
  ...
  unsigned long idbuf;
  unsigned int cidbuf;
  //    Real_mpi rbuf[3], vbuf[3] = {0., 0., 0.}, qbuf[4], Dbuf[3] = {0., 0.,
  //    0.};
  Real_mpi rbuf[3]{0, 0, 0};
#ifdef LS1_SEND_UNIQUE_ID_FOR_HALO_COPIES
  i_runningByte = readValue(i_runningByte, idbuf);
#else
  idbuf = UINT64_MAX;
  ...
  //    i_runningByte = readValue(i_runningByte, qbuf[3]);
  //    Component* component = _simulation.getEnsemble()->getComponent(cidbuf);
  //    TODO WHAT？？？？？ m = Molecule(idbuf, component,           //TODO
  //    目前来看是问题出在这里！！！！！！！！！！！！导致接收取到了Halo
  //                 rbuf[0], rbuf[1], rbuf[2],
  //                 vbuf[0], vbuf[1], vbuf[2],
  //                 qbuf[0], qbuf[1], qbuf[2], qbuf[3],
  //                 Dbuf[0], Dbuf[1], Dbuf[2]
  m = Atom(idbuf, rbuf[0], rbuf[1], rbuf[2]);
  //  );
  ...
  if (i_runningByte - i_firstByte != _numBytesHalo) {
    GlobalLogger::error(
        "i_runningByte - i_firstByte != _numBytesHalo in reading ");
  }
}

void CommunicationBuffer::readForceMolecule(size_t indexOfMolecule,
                                            Atom &m) const {
  // some mardyn assert
...
size_t CommunicationBuffer::getStartPosition(ParticleType_t type,
                                             size_t indexOfMolecule) const {
  size_t ret = 0;
  ...
  if (type == ParticleType_t::LEAVING) {
    ...
    // the number of forces is NOT stored in the buffer, as they are sent on
    // their own!
    ret = indexOfMolecule * _numBytesForces;
    // additionally no leaving or halo molecules are transmitted, thus
    // _numLeaving, _numHalo are zero!
  }
